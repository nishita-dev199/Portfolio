...
const skillCategories = [
  {
    title: "Full Stack",
    skills: [
      { name: "React", icon: <SiReact size={44} /> },
      { name: "Next.js", icon: <SiNextdotjs size={44} /> },
      { name: "Redux", icon: <SiRedux  size={44} /> },
      { name: "Node.js", icon: <SiNodedotjs size={44} /> },
      { name: "Express", icon: <SiExpress size={44} /> },
      { name: "REST APIs", icon: <RestApiIcon size={44} /> },
      { name: "Tailwind CSS", icon: <SiTailwindcss size={44} /> },
  ...
  },
  {
    title: "Databases",
    skills: [
      { name: "MongoDB", icon: <SiMongodb size={44} /> },
      { name: "MySQL", icon: <SiMysql  size={44} /> },
      { name: "PostgreSQL", icon: <SiPostgresql size={44} /> },
      { name: "Redis", icon: <SiRedis size={44} /> },
    ]
  },
  {
    title: "Languages",
    skills: [
      { name: "JavaScript (ES6+)", icon: <SiJavascript size={44} /> },
      { name: "TypeScript", icon: <SiTypescript size={44} /> },
      { name: "SQL", icon: <FaDatabase size={44} /> },
      { name: "Java", icon: <FaJava size={48} /> },
      { name: "Python", icon: <SiPython size={44} /> }, 
    ]
  },
  {
    title: "Tools & Infrastructure",
    skills: [
      { name: "AWS (EC2, RDS, S3, IAM, CloudWatch)", icon: <FaAws size={44} /> },
      { name: "Docker", icon: <SiDocker size={44} /> },
      { name: "Nginx", icon: <SiNginx size={44} /> },
      { name: "Git & GitHub", icon: <SiGit size={44} /> },
      { name: "Jest", icon: <SiJest size={44} /> },
      { name: "Postman", icon: <SiPostman size={44} /> },
    ]
  }
];
...
export default function Skills() {
  return (
    <section id="skills" className="py-24 px-6 md:px-12 relative font-inter min-h-screen flex flex-col items-center overflow-hidden">
      ...
      <div className="container mx-auto max-w-[1600px] relative z-10 flex flex-col gap-16">
        
        {skillCategories.map((category, idx) => (
          <div key={idx} className="flex flex-col items-center">
            <h3 className="text-[1.75rem] font-bold text-white mb-8 text-center tracking-wide">
              {category.title}
            </h3>
            ...
            <div className="flex flex-wrap justify-center gap-5">
              {category.skills.map((skill, i) => (
                <motion.div
                  ...
                  whileInView={{ opacity: 1, y: 0, transition: { duration: 0.3, delay: i * 0.02 } }}
                  whileHover={{ scale: 1.07, y: -10, transition: { duration: 0.2 } }}
                  viewport={{ once: true, margin: "-100px" }}
                  className="w-[140px] h-[140px] bg-[#111111]/80 backdrop-blur-sm border border-white/10 rounded-2xl flex flex-col items-center justify-center gap-4 hover:border-purple-light/50 shadow-lg shadow-black/50 cursor-default"
                >
                  <div className="text-purple-light">
                    {skill.icon}
                  </div>
                  <span className={`text-white font-bold tracking-wide text-center px-2 ${skill.name.length > 20 ? 'text-[11px] leading-tight' : 'text-sm'}`}>
                    {skill.name}
                  </span>
                </motion.div>
              ))}
